...
Let's now make a change to `mq01` and watch the build and deploy process happen
automatically.
...
The `mq01` queue manager running in the cluster is ultimately defined by the
`mq01-src` repository. Let's update this repository to add a new queue.

Copy the URL generated by the following command into your browser:

```bash
echo https://github.com/${GITORG}/mq01-src/blob/main/user/mqsc/mq01.mqsc
```

to view the MQSC file that is used by `mq01` to define its MQ objects:
...
Using the GitHub editor, **add** a new queue to the MQSC file as follows:

```text
DEFINE QLOCAL(IBM.THIS.QUEUE.FOR.YOU) REPLACE
```

and hit the `Commit changes` button (twice) to commit your changes.

You've now added a new queue to the `mq01`, and just as importantly, a webhook
event has been generated that describes this change.

---

## Examine GitHub webhook event

Let's explore the GitHub webhook event generated by this change:

Copy the URL generated by the following command into your browser:

```bash
https://github.com/$GITORG/mq01-src/settings/hooks
```

Navigate to the `Recent Deliveries` tab:

![diagram23](../images/diagram23.png)
